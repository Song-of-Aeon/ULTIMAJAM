...
new mapsetting("tiles", [tl, ["lol"], ["background", "behind", "main", "front", "foreground"]], function() {
	...
	draw_text_pointy(xdraw, ydraw+z*interval, opos[0]);
	draw_set_alpha(1);
	...
	draw_set_alpha(1);
	draw_set_color(pos==z++ ? c_white : c_aolu);
	draw_text_pointy(xdraw, ydraw+z*interval, options[2][opos[2]]);
	draw_set_alpha(1);
	...
	iterate options[1] to {
		...
		if mouse_within(palette.x+i%rowamount*16, palette.y+floor(i/rowamount)*16, palette.x+i%rowamount*16+16, palette.y+floor(i/rowamount)*16+16) && click.hit {
			opos[1] = i;
		}
	}
...
new mapsetting("entities", [["enemies"], ["lol"], ["enabled", "disabled"]], function() {
	...
	var z=0;
	draw_set_color(pos==z++ ? c_white : c_aolu);
	draw_text_pointy(xdraw, ydraw+z*interval, options[0][opos[0]]);
...
new mapsetting("triggers", [["tra", "touch event", "set spawn"], variable_struct_get_names(mp), array_create_order(100), array_create_order(100)], function() {
	...
	var z=0;
	draw_set_color(pos==z++ ? c_white : c_aolu);
	draw_text_pointy(xdraw, ydraw+z*interval, options[z-1][opos[z-1]]);
	draw_set_color(pos==z++ ? c_white : c_aolu);
	draw_text_pointy(xdraw, ydraw+z*interval, options[z-1][opos[z-1]]);
	draw_set_color(pos==z++ ? c_white : c_aolu);
	draw_text_pointy(xdraw, ydraw+z*interval, options[z-1][opos[z-1]]);
...
new mapsetting("map settings", [["name", "size", "spawn"], ["lol"], ["lol"]], function() {
	...
	if !opos[0] {
		...
		//draw_set_alpha((pos==z++)+.4);
		draw_set_color(pos==z++ ? c_white : c_aolu);
		draw_text_pointy(xdraw, ydraw+z*interval, options[1][opos[1]]);
		//draw_set_alpha((pos==z++)+.4);
		draw_set_color(pos==z++ ? c_white : c_aolu);
		draw_text_pointy(xdraw, ydraw+z*interval, options[2][opos[2]]);
	} else {
		...
		draw_text_pointy(xdraw, ydraw+2*interval, o_mapper.spawn.x/global.tilesize);
		draw_text_pointy(xdraw, ydraw+3*interval, o_mapper.spawn.y/global.tilesize);
	}
	draw_set_alpha(1);
}),
