...
function c_shoot(x, y, speed, direction, type, color=c_white,  behavior=c_null, draw_=u, additive_=type==bul.particle, silent=false) {
	...
	/*if type.friendly != false {
		chump = instance_create_layer(x, y, layer_get_id(!additive_ ? "friendly" : "friendly_add"), o_bullet); //no more
	} else {
		chump = instance_create_layer(x, y, layer_get_id(!additive_ ? "bullets" : "bullets_add"), o_bullet); //no more
	}*/
	chump = instance_create_layer(x, y, layer_get_id("instances"), o_bullet); //no more
	...
	if draw_ == u {
		if type.draw == c_bulletdraw && additive_ {
	...
	chump.originx = x;
	chump.originy = y;
	//chump.owner = chump.friendly ? me : self;
	chump.owner = self;
	
	if chump.friendly {
		tag("friendlyshot", chump);
		c_dohook(hooks.onplayershoot, [me, chump]);
	}
...
function c_shootfan(x, y, speed, startdirection, enddirection, amount, offsetamount, bullettype, color=c_white, behavior=c_null, draw=u, additive_=false, silent=false) {
	var i;
	var bulletcount = 0;
	var bullets = [];
	//offsetamount = speed;
