...
#macro spit \
for (var loggable;; {log(loggable) break}) \
	loggable =
...
#macro blend \
for (;; {gpu_set_blendmode(bm_normal); break}) \
	for (;; break) { \
		for (var bm;; {gpu_set_blendmode(bm); break}) \
			bm = 
...
#macro tetra for (l=0; l<array_length(array_get(
#macro monster , k)); l++)

var dudes = [[[[]]]];
iterate dudes to {
	double dudes gamble {
		triple dudes trouble {
			tetra dudes monster {
				dudes[i][j][k][l] = true;
			}
		}
...
/*for (var bm;; {log("first one")}) {
	for (var bm2;; {log("second one"); break})
		bm =*/
