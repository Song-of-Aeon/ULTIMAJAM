...
function c_lase(x, y, speed, direction, length, width, type, color=c_white, behavior=c_null, distanceBetweenLinks=5, escapedistance=200){
	...
	if chump.friendly c_dohook(hooks.onplayershoot, [me, chump]);
	
	return chump;
}

function c_lasecircle(x, y, speed, length, width, origindirection, density, archetype, color=c_white, behavior=c_null, draw=u, additive_=false, distance=0, silent=false, spddeviation=0) {
	var i;
	var bulletcount = 0;
	var bullets = [];
	...
	for (i=0; i<360; i+=360/density) {
		...
		var chump = c_lase(x+distguy.x, y+distguy.y, speed, origindirection+i, length, width, archetype, color, behavior, draw, additive_, silent);
